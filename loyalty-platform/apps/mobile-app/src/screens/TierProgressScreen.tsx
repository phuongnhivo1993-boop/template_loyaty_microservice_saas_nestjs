...
export default function TierProgressScreen() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      setLoading(false);
      if (!profile) setError('Failed to load profile data');
    }, 10000);
    return () => clearTimeout(timeout);
  }, [profile]);

  if (loading) return <LoadingState />;
  if (error) return <ErrorState message={error} onRetry={() => { setError(''); setLoading(true); }} />;
  if (!profile) return <ErrorState message="Profile not available" onRetry={() => { setError(''); setLoading(true); }} />;
  ...
  return (
    ...
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      <Text style={[styles.title, { color: colors.text }]}>Tier Progress</Text>

      <View style={[styles.currentTierCard, { backgroundColor: colors.card, borderColor: tierColor }]}>
        <Text style={[styles.currentTierName, { color: tierColor }]}>{currentTier}</Text>
        <Text style={[styles.pointsText, { color: colors.textSecondary }]}>{totalPoints.toLocaleString()} points</Text>
      </View>
      ...
      <View style={styles.progressSection}>
        ...
        <Text style={[styles.progressText, { color: colors.textSecondary }]}>{Math.round(clampedProgress)}% to next tier</Text>
      </View>
      ...
      {tiers.map((tier, i) => {
        ...
        return (
          <View key={tier.name} style={[styles.tierRow, { backgroundColor: colors.card, borderColor: colors.border }, isCurrent && { backgroundColor: tier.color + '11', borderColor: tier.color }]}>
            <View style={[styles.tierDot, { backgroundColor: tier.color }]} />
            <View style={{ flex: 1 }}>
              <Text style={[styles.tierName, { color: colors.text }, isCurrent && { color: tier.color, fontWeight: '800' }]}>{tier.name}</Text>
              <Text style={[styles.tierRange, { color: colors.textSecondary }]}>{tier.min.toLocaleString()} - {tier.max.toLocaleString()} points</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  currentTierCard: { marginHorizontal: 20, borderRadius: 20, padding: 24, borderWidth: 2, alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 8, elevation: 2 },
  ...
  tierRow: { flexDirection: 'row', alignItems: 'center', marginHorizontal: 20, marginBottom: 8, padding: 16, borderRadius: 14, borderWidth: 1 },
  tierDot: { width: 14, height: 14, borderRadius: 7, marginRight: 14 },
  tierName: { fontSize: 16, fontWeight: '600' },
  tierRange: { fontSize: 13, marginTop: 2 },
