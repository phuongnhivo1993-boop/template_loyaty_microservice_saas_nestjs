var qrcode = function() {
  var qrcode = function(typeNumber, errorCorrectionLevel) {
    ...
    _this.createSvgTag = function(cellSize, margin) {
      ...
      var qrSvg = '<?xml version="1.0" encoding="UTF-8"?>';
      qrSvg += '<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">';
      qrSvg += '<svg xmlns="http://www.w3.org/2000/svg" width="' + size + '" height="' + size + '" viewBox="0 0 ' + size + ' ' + size + '">';
      qrSvg += '<rect width="100%" height="100%" fill="white"/>';
      for (var r = 0; r < _this.getModuleCount(); r += 1) {
        for (var c = 0; c < _this.getModuleCount(); c += 1) {
          if (_this.isDark(r, c)) {
            qrSvg += '<rect x="' + (margin + c * cellSize) + '" y="' + (margin + r * cellSize) + '" width="' + cellSize + '" height="' + cellSize + '" fill="black"/>';
          }
        }
      }
      qrSvg += '</svg>';
  ...
  var QRUtil = function() {
    var PATTERN_POSITION_TABLE = [[], [6, 18], [6, 22], [6, 26], [6, 30], [6, 34], [6, 22, 38], [6, 24, 42], [6, 26, 46], [6, 28, 50], [6, 30, 54], [6, 32, 58], [6, 34, 62], [6, 26, 46, 66], [6, 26, 48, 70], [6, 26, 50, 74], [6, 30, 54, 78], [6, 30, 56, 82], [6, 30, 58, 86], [6, 34, 62, 90], [6, 28, 50, 72, 94], [6, 26, 50, 74, 98], [6, 30, 54, 78, 102], [6, 28, 54, 80, 106], [6, 32, 58, 84, 110], [6, 30, 58, 86, 114], [6, 34, 62, 90, 118], [6, 26, 50, 74, 98, 122], [6, 30, 54, 78, 102, 126], [6, 26, 52, 78, 104, 130], [6, 30, 56, 82, 108, 134], [6, 34, 60, 86, 112, 138], [6, 30, 58, 86, 114, 142], [6, 34, 62, 90, 118, 146], [6, 30, 54, 78, 102, 126, 150], [6, 24, 50, 76, 102, 128, 154], [6, 28, 54, 80, 106, 132, 158], [6, 32, 58, 84, 110, 136, 162], [6, 26, 52, 78, 104, 130, 156, 182], [6, 26, 48, 70, 92, 114, 136, 158, 180]];
    ...
    _this.getLostPoint = function(qrcode) {
      ...
      var darkCount = 0;
      for (var col = 0; col < moduleCount; col += 1) {
        for (var row = 0; row < moduleCount; row += 1) { if (qrcode.isDark(row, col)) darkCount += 1; }
      }
  ...
  function qrPolynomial(num, shift) {
    ...
    _this.multiply = function(e) {
      ...
      for (var i = 0; i < _this.getLength(); i += 1) {
        for (var j = 0; j < e.getLength(); j += 1) { num[i + j] ^= QRMath.gexp(QRMath.glog(_this.getAt(i)) + QRMath.glog(e.getAt(j))); }
      }
  ...
  var QRRSBlock = function() {
    ...
    var getRsBlockTable = function(typeNumber, errorCorrectionLevel) {
      ...
      case QRErrorCorrectionLevel.L: return RS_BLOCK_TABLE[4 * (typeNumber - 1) + 0];
      case QRErrorCorrectionLevel.M: return RS_BLOCK_TABLE[4 * (typeNumber - 1) + 1];
      case QRErrorCorrectionLevel.Q: return RS_BLOCK_TABLE[4 * (typeNumber - 1) + 2];
      case QRErrorCorrectionLevel.H: return RS_BLOCK_TABLE[4 * (typeNumber - 1) + 3];
  ...
  function qrAlphaNum(data) {
    ...
    var getCode = function(c) {
      ...
      else if (' $%*+-./:'.indexOf(c) >= 0) return ' $%*+-./:'.indexOf(c) + 36;
      else throw new Error('illegal char: ' + c);
    };
    return _this;
  }
  ...
  function qr8BitByte(data) {
    ...
    var _data = qrcode.stringToBytes(data);
    var _this = {};
    _this.getMode = function() { return _mode; };
    _this.getLength = function() { return _data.length; };
    _this.write = function(buffer) {
      for (var i = 0; i < _data.length; i += 1) { buffer.put(_data[i], 8); }
    };
    return _this;
  }
  ...
  function qrKanji(data) {
    ...
    _this.write = function(buffer) {
      ...
      var i = 0;
      while (i + 1 < s.length) {
        var c = ((s.charCodeAt(i) << 8) | s.charCodeAt(i + 1));
        if (0x8140 <= c && c <= 0x9FFC) c -= 0x8140;
        else if (0xE040 <= c && c <= 0xEBBF) c -= 0xC140;
        else throw new Error('illegal char at ' + (i + 1) + '/' + (c));
...
const QRCodeConstructor = qrcode();
export default QRCodeConstructor;