...
export default function BadgesPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/badges/${id}`, { method: 'DELETE', headers });
    load();
  };
  ...
  const modal = showModal && (
    <div style={{
      ...
      <div style={{ background: 'white', borderRadius: '12px', padding: '32px', width: '520px', maxHeight: '80vh', overflow: 'auto' }}
        ...
        <h2 style={{ fontSize: '20px', fontWeight: 700, marginBottom: '20px' }}>{editing ? 'Edit Badge' : 'New Badge'}</h2>
    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <main style={{ flex: 1, padding: '32px', marginLeft: '260px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <div>
            <h1 style={{ fontSize: '28px', fontWeight: 700 }}>Badges</h1>
            <p style={{ color: '#64748b' }}>Manage achievement badges</p>
          </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f8fafc', textAlign: 'left' }}>
                ...
                <th style={{ padding: '12px 16px', fontWeight: 600, fontSize: '14px', color: '#64748b' }}>Icon</th>
                <th style={{ padding: '12px 16px', fontWeight: 600, fontSize: '14px', color: '#64748b' }}>Description</th>
            ...
            <tbody>
              {badges.length === 0 ? (
                <tr><td colSpan={5} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>No badges found</td></tr>
              ) : badges.map((c: any) => (
                <tr key={c.id} style={{ borderTop: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '12px 16px', fontWeight: 500 }}>{c.name}</td>
                  <td style={{ padding: '12px 16px' }}>
                    {c.iconUrl ? <img src={c.iconUrl} alt="" style={{ width: '32px', height: '32px', borderRadius: '6px' }} /> : '-'}
                  </td>
                  <td style={{ padding: '12px 16px', color: '#64748b', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.description || '-'}</td>
