...
export default function TiersPage() {
  ...
  const columns = [
    ...
    { key: 'minPoints', label: 'Min Points', render: (t: any) => <span className="text-muted">{t.minPoints?.toLocaleString()}</span> },
    { key: 'maxPoints', label: 'Max Points', render: (t: any) => <span className="text-muted">{t.maxPoints?.toLocaleString()}</span> },
    { key: 'benefits', label: 'Benefits', render: (t: any) => <span className="text-muted" style={{ maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'inline-block' }}>{t.benefits}</span> },
    ...
    { key: 'status', label: 'Status', render: (t: any) => (
      <span className={`status-badge ${t.status === 'ACTIVE' ? 'active' : ''}`}>{t.status}</span>
    )},
    ...
    <div className="page-layout">
      ...
      <main className="main-content">
        <PageHeader
          ...
          actions={<button onClick={openCreate} className="btn-primary">+ New Tier</button>}
        />
        ...
        <Modal open={showModal} title={editing ? 'Edit Tier' : 'New Tier'} onClose={() => setShowModal(false)}>
          <form onSubmit={handleSubmit}>
            ...
            <FormInput label="Color" value={form.color} onChange={e => setForm({ ...form, color: e })} type="color" />
            <FormSelect label="Status" value={form.status} onChange={e => setForm({ ...form, status: e })} options={[{ value: 'ACTIVE', label: 'Active' }, { value: 'INACTIVE', label: 'Inactive' }]} />
